...
//Extended for using all decryption capabilities offered from the hardware
...
//Fahad: Enable Debug Messages
#define DEBUG_ENABLED 		0

#define XDCFG_DEBUG(...) if (DEBUG_ENABLED)  { printk(__VA_ARGS__); }
...
#define XDCFG_CONFIG_OFFSET		0x08 /* Configure Register */
...
#define XDCFG_ERROR_REBOOT_REQ 02 /* Reboot Req Error Code */
...
#define XDCFG_IXR_ERROR_FLAGS_MASK	0x00F0C860
#define XDCFG_IXR_ALL_ERROR_FLAGS_MASK  0xFFFFFFFF /* Mask all Errors */
...
/**
 * xdevcfg_int_status - Checks the INT Status register and updates state
 * @data:	Pointer to the driver data structure.
 **/
static void xdevcfg_int_status(void *data)
{
	u32 intr_status;
	...
	XDCFG_DEBUG("Int Status: %d",intr_status);
	
	/* Clear the interrupts */
	xdevcfg_writereg(drvdata->base_address + XDCFG_INT_STS_OFFSET,
				intr_status);

	if ((intr_status & XDCFG_IXR_ERROR_FLAGS_MASK) > 0)
	{
		XDCFG_DEBUG("Error");
		...
		loff_t *ppos)
{
	...
	u32 intr_reg, dma_len, dest_len;
	dma_addr_t dma_addr, dma_addr_dest;
	u32 transfer_length = 0;
	u32 transfer_length_dest = 0;
	struct xdevcfg_drvdata *drvdata = file->private_data;
	size_t user_count = count;
	int i;
	u32 ctrlreg, aes_en;
	
	//Fahad: Return instantly if Reboot Flag Set
	if(drvdata->reboot_req){
		return -EHOSTDOWN;
	}
	...
	//Fahad : loopback changes 
	if(drvdata->is_loopback){
		//Fahad: the first 4 bytes of count contain  the destination lengh in bytes
		char temp_buffer [4];
		if (copy_from_user(&temp_buffer , buf, 4)) {
			status = -EFAULT;
			goto err_clk;
		}
		dest_len = temp_buffer[0]  |  ( (u32)temp_buffer[1] << 8 )  |  ( (u32)temp_buffer[2] << 16 )  |  ( (u32)temp_buffer[3] << 24 );
		count = count - 4 ;
		dma_len = count  ;
	}
	else{
	...
	//Fahad : dma_alloc_coherent allocates a buffer in physical memory without virtual memory addresses which may be used in usual buffers
	...
	if(drvdata->is_loopback){
		...
		//Fahad: Additional Destination buffer is needed in the case of loopback mode
		kbuf_dest = dma_alloc_coherent(drvdata->dev, dest_len, &dma_addr_dest, GFP_KERNEL);		
		if (!kbuf_dest) {
			status = -ENOMEM;
			goto err_unlock_2;
		}
		//Fahad: Memory copy user data to allocated buffer
		if (copy_from_user(kbuf , buf, count)) {
			status = -EFAULT;
			goto error;
		}

		//Fahad: Check AES Status Register
		aes_en = xdevcfg_readreg(drvdata->base_address +
				XDCFG_CTRL_OFFSET) & XDCFG_CTRL_PCFG_AES_EN_MASK;

		if(!aes_en){
			//Fahad: Enable PCAP Loopback for non secure loopback
	...
	//Fahad : For now it is assumed the data will be be word alligned and header is not taken care off
	/* Enable DMA and error interrupts */
	xdevcfg_writereg(drvdata->base_address + XDCFG_INT_STS_OFFSET,
				XDCFG_IXR_ALL_ERROR_FLAGS_MASK);
	...
	xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_SRC_ADDR_OFFSET, (u32)(dma_addr | 1));
	
	//Fahad: two different destinations for loopback and fpga respectively
	if(drvdata->is_loopback){
		//Fahad: Give the destination address for the destination buffer
	...
	//Fahad: Setting the destination length starts the DMA Transfer
	if(drvdata->is_loopback){
		transfer_length_dest = dest_len / 4; //Fahad: Converting to Word
		xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_DEST_LEN_OFFSET, transfer_length_dest);	
	}
	else{
		xdevcfg_writereg(drvdata->base_address + XDCFG_DMA_DEST_LEN_OFFSET, 0);
	}

	//Fahad: Maybe need to increase it to accomodate AES Dycryption time later 
	timeout = jiffies + msecs_to_jiffies(2000);

	while (!READ_ONCE(drvdata->dma_done)) {
		//Fahad: Check INT Status
		xdevcfg_int_status(drvdata);
		if (time_after(jiffies, timeout)) {
			//Fahad: Error: Raise the Reboot Req Flag
			drvdata->reboot_req = 1;
			status = -EHOSTDOWN;
			XDCFG_DEBUG(KERN_ERR "timed out!\n");
			goto error;
		}
	}
	...
	{
		//Fahad: Raise the Reboot Req Flag
		XDCFG_DEBUG("Reboot Flag: Enable");
		drvdata->reboot_req = 1;
	...
	*ppos += user_count;
	...
	//Fahad: In case of loopback mode we need to copy contents from the dma destination buffer to the user buffer
	//Warning: Check size!!
	if(drvdata->is_loopback){
		if (copy_to_user(buf, kbuf_dest, dest_len)) {
			status = -EFAULT;
		}
		else{
			status = dest_len;
...
error:
	if(drvdata->is_loopback){
		//Fahad: Free the DMA buffer for destination in loopback case
		dma_free_coherent(drvdata->dev, dma_len, kbuf_dest, dma_addr_dest);
		//Fahad: Ensure internal PCAP loopback is disabled for regular FPGA Usage
		ctrlreg = xdevcfg_readreg(drvdata->base_address + XDCFG_MCTRL_OFFSET);
		xdevcfg_writereg(drvdata->base_address + XDCFG_MCTRL_OFFSET,
					(~XDCFG_MCTRL_PCAP_LPBK_MASK &
					ctrlreg));
...
static int xdevcfg_open(struct inode *inode, struct file *file)
{
	...
	//Fahad : Adding new branch which will skip through the usual fpga bitstream loading stream
	if (drvdata->is_loopback) {
		//Do something
	}
	else{
		...
		//TODO: Fahad: Maybe we should do it in the write function
		xdevcfg_writereg(drvdata->base_address + XDCFG_INT_STS_OFFSET,
...
static int xdevcfg_release(struct inode *inode, struct file *file)
{
	...
	//Fahad : Adding additional branch to work with loopback feature
	if(drvdata->is_loopback){
		//Fahad : Do Something here
	}
	else{
		if (drvdata->is_partial_bitstream)
...
/*
Fahad : Device Set and Show functions for enabling and disabling device loopback feature
*/
static ssize_t xdevcfg_set_is_loopback(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long mask_bit;
	ssize_t status;
	...
	/* kstrtoul converts the null terminated string in the buffer to a number from the selected base. Here 10 for Decimal */
	status = kstrtoul(buf, 10, &mask_bit);
	...
	if (mask_bit)
		drvdata->is_loopback = 1;
	else
		drvdata->is_loopback = 0;

	return size;
}

static ssize_t xdevcfg_show_is_loopback(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t status;
	struct xdevcfg_drvdata *drvdata = dev_get_drvdata(dev);
...
static DEVICE_ATTR(is_loopback, 0644,
				xdevcfg_show_is_loopback,
				xdevcfg_set_is_loopback);

/*
* Fahad: Reboot Request status show
*/

static ssize_t xdevcfg_show_reboot_req(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t status;
	struct xdevcfg_drvdata *drvdata = dev_get_drvdata(dev);
...
static DEVICE_ATTR(reboot_requested, 0644,
				xdevcfg_show_reboot_req, NULL);
...
static const struct attribute *xdevcfg_attrs[] = {
	...
	&dev_attr_is_loopback.attr, /* Flag for loopback feature */
	&dev_attr_reboot_requested.attr, /* Show the reboot requested flag*/
	NULL,
};
...
/*
Fahad: For now, no interrupt is being registered
...
*/
	...
	//Fahad : Change from ICAP to PCAP, we need PCAP as default for loopback decryption.
	ctrlreg |= XDCFG_CTRL_PCAP_PR_MASK;
